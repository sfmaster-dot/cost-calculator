...
// ── 상수 ──────────────────────────────────────────────────────────────────────
const STORE_COLORS = [
  "#f5c842","#ff6b35","#3dd68c","#60a5fa",
  "#c084fc","#f472b6","#fb923c","#34d399",
];
const TODAY = new Date();

function genId() { return Math.random().toString(36).slice(2, 9); }
function calcCost(ings: Ingredient[]) {
  return ings.reduce((s, i) => s + (i.amount / 100) * i.unitPrice, 0);
}
...
export default function CostApp() {
  ...
  async function handleAddStore() {
    ...
    setStores(s);
    setNewStoreName(""); setAddingStore(false);
    toast("✅ 매장이 추가됐습니다");
  }
  ...
  async function handleAddMenu() {
    ...
    const ref = await saveMenu(user.uid, selectedStore.id, newMenu);
  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "32px 16px 80px" }}>
      ...
      <div style={{ display:"flex", gap:4, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:4, marginBottom:24 }}>
        {(["calc","summary","reverse"] as const).map((t, i) => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            fontFamily:"'Noto Sans KR',sans-serif", fontSize:13, fontWeight: tab===t ? 700 : 500,
            cursor:"pointer",
          }}>
      ...
      {/* 탭 콘텐츠 */}
      {tab === "calc" && (
        <CalcPanel menus={menus} onChange={handleMenuChange} onAdd={handleAddMenu} onDelete={handleDeleteMenu} />
      )}
      {tab === "summary" && (
        <SummaryPanel menus={menus} />
      )}
      {tab === "reverse" && (
        <ReversePanel />
      )}
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"100vh", gap:24, padding:24 }}>
      ...
      <div style={{ textAlign:"center" }}>
        <div style={{ fontSize:24, fontWeight:900, marginBottom:8 }}>
          단꿈 <span style={{ color:"var(--accent)" }}>원가율</span> 계산기
        </div>
        <div style={{ fontSize:14, color:"var(--text-sub)", lineHeight:1.7 }}>
          매장별 메뉴 원가율을 관리하고<br />손익분석기와 연동하세요
        </div>
      </div>
      <button onClick={onLogin} style={{
        ...
        fontSize:14, fontWeight:700, fontFamily:"'Noto Sans KR',sans-serif", cursor:"pointer",
      }}>
  ...
  return (
    <div style={{ maxWidth: 560, margin: "0 auto", padding: "40px 16px 80px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:32 }}>
        <div>
          ...
          <div style={{ fontSize:22, fontWeight:900 }}>
            내 <span style={{ color:"var(--accent)" }}>매장</span> 선택
          </div>
        </div>
      ...
      {stores.length === 0 && !addingStore && (
        <div style={{ textAlign:"center", padding:"48px 0", color:"var(--text-sub)", fontSize:14, lineHeight:1.8 }}>
          아직 등록된 매장이 없어요.<br/>아래 버튼으로 첫 매장을 추가해보세요!
        </div>
      )}
        ...
        {stores.map((store: Store) => (
          ...
          >
            ...
            <button
              ...
              style={{ position:"absolute", top:10, right:10, background:"transparent", border:"none", color:"var(--text-sub)", fontSize:16, cursor:"pointer", padding:4 }}
            >✕</button>
          </div>
        ))}
      </div>
      ...
      {addingStore ? (
        <div style={S.card}>
          ...
          <input
            ...
            placeholder="매장 이름 (예: 삼겹살집 홍대점)"
            value={newStoreName}
            onChange={e => setNewStoreName(e.target.value)}
            onKeyDown={e => e.key === "Enter" && onAddStore()}
            autoFocus
          />
          <div style={{ display:"flex", gap:8, marginBottom:16 }}>
            {STORE_COLORS.map(c => (
              <div key={c} onClick={() => setNewStoreColor(c)} style={{
                width:28, height:28, borderRadius:"50%", background:c, cursor:"pointer",
                outline: newStoreColor===c ? `3px solid white` : "none",
                outlineOffset:2,
              }} />
            ))}
          </div>
        ...
        <button
          ...
          style={{ width:"100%", padding:14, borderRadius:"var(--radius)", border:"1px dashed var(--border)", background:"transparent", color:"var(--text-sub)", fontFamily:"'Noto Sans KR',sans-serif", fontSize:14, cursor:"pointer" }}
        >
          ＋ 매장 추가
        </button>
      )}
...
// ── 원가 계산 패널 ─────────────────────────────────────────────────────────────
function CalcPanel({ menus, onChange, onAdd, onDelete }: {
  menus: Menu[];
  ...
  return (
    <div>
      <div style={{ background:"rgba(245,200,66,0.07)", border:"1px solid rgba(245,200,66,0.2)", borderRadius:"var(--radius)", padding:"14px 18px", fontSize:13, color:"var(--text-sub)", lineHeight:1.7, marginBottom:20 }}>
        ...
        메뉴명과 판매가 → 재료별 1인분 사용량(g)과 100g당 단가 입력<br />
      ...
      {menus.length === 0 && (
        <div style={{ textAlign:"center", padding:"40px 0", color:"var(--text-sub)", fontSize:14 }}>
          아직 메뉴가 없어요. 아래 버튼으로 추가해보세요!
        </div>
      )}
...
function MenuCard({ menu, colorIdx, onChange, onDelete }: {
  menu: Menu; colorIdx: number;
  onChange: (m: Menu) => void;
  onDelete: (id: string) => void;
}) {
  ...
  function updateIng(idx: number, field: keyof Ingredient, val: string) {
    ...
    (ings[idx] as any)[field] = field==="name" ? val : (parseFloat(val)||0);
    onChange({ ...menu, ingredients: ings });
  }

  function addIng() {
    onChange({ ...menu, ingredients: [...(menu.ingredients||[]), { id:genId(), name:"", amount:0, unitPrice:0 }] });
  }
  ...
  return (
    <div style={{ ...S.card, borderLeft:`3px solid ${color}` }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
        <div>
          <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.05em", marginBottom:5 }}>판매가 (원)</div>
          <div style={{ position:"relative" }}>
            <input type="number" style={{ ...S.input, fontFamily:"'DM Mono',monospace", paddingRight:32 }}
              placeholder="15000" value={menu.price||""}
        ...
        <div>
          <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.05em", marginBottom:5 }}>목표 원가율 (%)</div>
          <div style={{ position:"relative" }}>
      ...
      {/* 재료 목록 */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 80px 90px 32px", gap:6, marginBottom:6 }}>
        {["재료명","사용량(g)","100g단가",""].map((h,i) => (
          <div key={i} style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.04em", paddingLeft:i<3?8:0 }}>{h}</div>
        ))}
      </div>
      {(menu.ingredients||[]).map((ing, idx) => (
        <div key={ing.id} style={{ display:"grid", gridTemplateColumns:"1fr 80px 90px 32px", gap:6, marginBottom:6, alignItems:"center" }}>
          <input style={S.input} placeholder="예: 아귀" value={ing.name}
            onChange={e => updateIng(idx, "name", e.target.value)} />
          <input type="number" style={{ ...S.input, textAlign:"right", fontFamily:"'DM Mono',monospace" }}
            placeholder="200" value={ing.amount||""}
            onChange={e => updateIng(idx, "amount", e.target.value)} />
          <input type="number" style={{ ...S.input, textAlign:"right", fontFamily:"'DM Mono',monospace" }}
            placeholder="1500" value={ing.unitPrice||""}
            onChange={e => updateIng(idx, "unitPrice", e.target.value)} />
          ...
          {rate > 0 && (
            <div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"var(--text-sub)" }}>
                <span>0%</span>
                <span style={{ color:"var(--accent)" }}>목표 {target}%</span>
                <span>60%+</span>
              </div>
            </div>
          )}
...
function SummaryPanel({ menus }: { menus: Menu[] }) {
  ...
  const avgRate = withData.length > 0
    ? withData.reduce((s, m) => s + calcRate(calcCost(m.ingredients||[]), m.price), 0) / withData.length
    : 0;
        ...
        <div style={S.card}>
                ...
                {menus.map((m, i) => {
                  ...
                  return (
                    <tr key={m.id}>
                      ...
                      <td style={{ padding:"11px 10px", borderBottom:"1px solid var(--border)" }}>
                        {rate > 0 ? (
                          <span style={{ display:"inline-block", padding:"3px 10px", borderRadius:99, fontFamily:"'DM Mono',monospace", fontSize:12, background:`${rateColor(rate)}22`, color:rateColor(rate) }}>
                            {rate.toFixed(1)}%
                          </span>
                        ) : "—"}
                      </td>
          ...
          {avgRate > 0 && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, background:"var(--surface2)", borderRadius:10, padding:"14px 16px", border:"1px solid var(--border)", marginTop:16 }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", marginBottom:4 }}>전체 메뉴 수</div>
                <div style={{ fontFamily:"'DM Mono',monospace", fontSize:20 }}>{withData.length}개</div>
              </div>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", marginBottom:4 }}>평균 원가율</div>
                <div style={{ fontFamily:"'DM Mono',monospace", fontSize:20, color:rateColor(avgRate) }}>{avgRate.toFixed(1)}%</div>
              </div>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", marginBottom:4 }}>평가</div>
                <div style={{ fontSize:16 }}>{rateLabel(avgRate)}</div>
              </div>
            </div>
          )}
        </div>
...
function ReversePanel() {
  ...
  const cost = calcCost(ings);
  const basic = targetRate > 0 && cost > 0 ? cost / (targetRate / 100) : 0;
  const full = targetRate > 0 && cost > 0
    ? (cost + packCost) / ((targetRate / 100) * (1 - delivFee / 100))
    : 0;
  const rounded = Math.ceil(full / 500) * 500;

  function addIng() { setIngs(p => [...p, { id:genId(), name:"", amount:0, unitPrice:0 }]); }
  function removeIng(idx: number) { setIngs(p => p.filter((_,i) => i!==idx)); }
  function updateIng(idx: number, field: keyof Ingredient, val: string) {
    setIngs(p => p.map((ing, i) => i===idx ? { ...ing, [field]: field==="name"?val:(parseFloat(val)||0) } : ing));
  }
  ...
  return (
    <div>
      <div style={{ background:"rgba(245,200,66,0.07)", border:"1px solid rgba(245,200,66,0.2)", borderRadius:"var(--radius)", padding:"14px 18px", fontSize:13, color:"var(--text-sub)", lineHeight:1.7, marginBottom:20 }}>
        ...
        식재료 원가 입력 → 목표 원가율 설정 → <strong style={{ color:"var(--text)" }}>최소 판매가 자동 계산</strong><br />
        배달 수수료·포장비까지 고려한 권장 판매가도 함께 알려드려요.
      </div>

      <div style={S.card}>
        <div style={{ fontSize:12, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.06em", marginBottom:14, textTransform:"uppercase" }}>식재료 원가 입력</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 80px 90px 32px", gap:6, marginBottom:6 }}>
          {["재료명","사용량(g)","100g단가",""].map((h,i) => (
            <div key={i} style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", paddingLeft:i<3?8:0 }}>{h}</div>
          ))}
        </div>
        {ings.map((ing, idx) => (
          <div key={ing.id} style={{ display:"grid", gridTemplateColumns:"1fr 80px 90px 32px", gap:6, marginBottom:6, alignItems:"center" }}>
            <input style={S.input} placeholder="재료명" value={ing.name} onChange={e => updateIng(idx,"name",e.target.value)} />
            <input type="number" style={{ ...S.input, textAlign:"right", fontFamily:"'DM Mono',monospace" }} placeholder="200" value={ing.amount||""} onChange={e => updateIng(idx,"amount",e.target.value)} />
            <input type="number" style={{ ...S.input, textAlign:"right", fontFamily:"'DM Mono',monospace" }} placeholder="1500" value={ing.unitPrice||""} onChange={e => updateIng(idx,"unitPrice",e.target.value)} />
            <button onClick={() => removeIng(idx)} style={{ width:32, height:34, borderRadius:6, border:"1px solid var(--border)", background:"transparent", color:"var(--text-sub)", fontSize:14, cursor:"pointer" }}>✕</button>
          </div>
        ))}
      ...
      <div style={S.card}>
        <div style={{ fontSize:12, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.06em", marginBottom:14, textTransform:"uppercase" }}>목표 설정</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.05em", marginBottom:5 }}>목표 원가율 (%)</div>
            <div style={{ position:"relative" }}>
              <input type="number" style={{ ...S.input, fontFamily:"'DM Mono',monospace", paddingRight:28 }}
                value={targetRate} onChange={e => setTargetRate(parseFloat(e.target.value)||30)} />
              <span style={{ position:"absolute", right:9, top:"50%", transform:"translateY(-50%)", fontSize:11, color:"var(--text-sub)" }}>%</span>
            </div>
          </div>
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.05em", marginBottom:5 }}>배달 수수료 (%)</div>
            <div style={{ position:"relative" }}>
              <input type="number" step="0.1" style={{ ...S.input, fontFamily:"'DM Mono',monospace", paddingRight:28 }}
                value={delivFee} onChange={e => setDelivFee(parseFloat(e.target.value)||0)} />
              <span style={{ position:"absolute", right:9, top:"50%", transform:"translateY(-50%)", fontSize:11, color:"var(--text-sub)" }}>%</span>
            </div>
          </div>
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-sub)", letterSpacing:"0.05em", marginBottom:5 }}>포장재·기타 (원)</div>
            <div style={{ position:"relative" }}>
              <input type="number" style={{ ...S.input, fontFamily:"'DM Mono',monospace", paddingRight:28 }}
                value={packCost} onChange={e => setPackCost(parseFloat(e.target.value)||0)} />
      ...
      <div style={{ background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:"24px 20px", textAlign:"center" }}>
        ...
        <div style={{ fontFamily:"'DM Mono',monospace", fontSize:34, color:"var(--accent)", marginBottom:16 }}>
          {basic > 0 ? `${fmt(Math.ceil(basic/500)*500)}원~` : "—"}
        </div>
        <div style={{ height:1, background:"var(--border)", margin:"0 0 16px" }} />
        <div style={{ fontSize:12, color:"var(--text-sub)", marginBottom:6 }}>배달 수수료·포장비 포함 권장 판매가</div>
        <div style={{ fontFamily:"'DM Mono',monospace", fontSize:26, color:"var(--text)", marginBottom:8 }}>
          {full > 0 ? `${fmt(rounded)}원~` : "—"}
        </div>
        {full > 0 && (
          <div style={{ fontSize:11, color:"var(--text-sub)" }}>
            식재료 {fmt(cost)}원 + 포장재 {fmt(packCost)}원 · 배달수수료 {delivFee}% 반영 · 500원 단위 올림
          </div>
        )}
      </div>
    </div>
...
function CopyMenuDropdown({ stores, onCopy }: { stores: Store[]; onCopy: (id:string) => void }) {
  ...
  return (
    <div style={{ position:"relative" }}>
      <button onClick={() => setOpen(p=>!p)} style={{ ...S.btn(), fontSize:12, padding:"7px 14px" }}>
        📋 메뉴 복사
      </button>
      {open && (
        <div style={{ position:"absolute", top:"calc(100% + 6px)", right:0, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:10, minWidth:180, zIndex:10, overflow:"hidden" }}>
          ...
          {stores.map(s => (
            <button key={s.id} onClick={() => { onCopy(s.id); setOpen(false); }} style={{
              width:"100%", padding:"10px 14px", border:"none", background:"transparent",
              color:"var(--text)", fontFamily:"'Noto Sans KR',sans-serif", fontSize:13,
